...
def survlime(explainer, features_df, num_neighbors=100, sample_id=0):
	...
	args = (model_chf, baseline_chf, neighbors, distance_weights, adjustment_weights, unique_times)
...
def generate_neighbors(explainer, explained_sample_df, num_neighbors):
	...
	neighbor_df = pd.DataFrame(data=np.random.normal(size=(num_neighbors, explained_sample_df.shape[1])), columns=fnames_ext)
	...
	for idx, fname in enumerate(feat_names):
		if fname in explainer.numeric_feat_names:
			...
			sampled_values = cate_feat_counts.sample(num_neighbors, replace=True, weights="counts", random_state=1)[cate_fname_ext].values
...
def compute_baseline_chf(survival_labels):
	...
	"""
	survival_times, survival_indicators = survival_labels[:, 0], survival_labels[:, 1].astype(bool)
...
def survlime_objective(x, model_chf, baseline_chf, neighbors, distance_weights, adjustment_weights, unique_times):
	"""
...
def format_survlime_results(features, coefficients, explained_sample):
	...
	return pd.DataFrame({"Feature": features, "Coefficient": coefficients, "Feature Importance": feature_importance})
