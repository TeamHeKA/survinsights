...
def partial_dependence_plots(
    ...
    ICE_df = individual_conditional_expectation(
        explainer, expl_f_name, num_samples, num_grid_points, prediction_type
    )

    PDP_df = (
        ICE_df.groupby([expl_f_name, "times"])
        .mean()
        .reset_index()[[expl_f_name, "times", "pred"]]
    )
...
def plot_pdp(explainer, pdp_results_df, ylim=None):
    ...
    explained_feature_name = next(
        col
        for col in pdp_results_df.columns.values
    ...
    if explained_feature_name in explainer.numeric_feat_names:
        ...
        cmap = mpl.cm.ScalarMappable(
            norm=mpl.colors.Normalize(0.0, max(unique_values), True), cmap="BrBG"
        )
        ...
        sns.lineplot(
            data=res_sorted, x="times", y="pred", hue=explained_feature_name, ax=ax
        )
