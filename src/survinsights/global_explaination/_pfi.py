...
def permutation_feature_importance(explainer, eval_times=None, num_perm=10,
								   loss_metric="brier_score", output_type="ratio"):
	"""
	...
	base_performance = performance.evaluate(
		explainer, features_df, survival_labels, times=eval_times, metric=loss_metric)["perf"].values
	...
	for feature_group in expanded_feats_name:
		...
		permuted_perf = get_permuted_performance(
			explainer, features_df, survival_labels, eval_times, feature_group, num_perm, loss_metric
		)
...
def get_permuted_performance(explainer, features_df, surv_labels, eval_times, feature_group, n_perm, metric):
	...
	for _ in range(n_perm):
		...
		permuted_perf += performance.evaluate(
				explainer, permuted_features_df, surv_labels, times=eval_times, metric=metric)["perf"].values / n_perm
