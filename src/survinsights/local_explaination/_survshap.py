...
def survshap(explainer, new_data, sample_id=0):
	...
	def preprocess_for_shap(feats_df):
		...
		for idx, f_name in enumerate(decoded_feat_names):
			if f_name in explainer.cate_feat_names:
				...
				preprocessed_feats_df[feat_col_sel] = feat_encoder.transform(feats_df[:, idx].reshape((-1, 1))).toarray()
			else:
	...
	for feat_name in explainer.cate_feat_names + explainer.numeric_feat_names:
		if feat_name in explainer.cate_feat_names:
			...
			decoded_features_df[feat_name] = encoder.inverse_transform(explainer.features_df[encoded_feat_name]).flatten()
