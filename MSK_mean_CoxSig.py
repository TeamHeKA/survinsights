...
for pred_time in pred_times:
    ...
    for i in range(K):
        tmp = survshap_longi(model_explainer, i, sel_paths[i], sel_static_feature[i], pred_time=np.array([pred_time]), 
                                        prediction_type="survival", seed=seed, n_split=4, intercept=True, reg=.1)
...
np.save('results/shap_MSK_mean_CoxSig.npy', np.array(all_shap, dtype=object), allow_pickle=True)