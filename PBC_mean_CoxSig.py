...
for pred_time in pred_times:
    ...
    for i in range(K):
        tmp = survshap_longi(model_explainer, i, sel_paths[i], sel_static_feature[i], pred_time=np.array([pred_time]), 
                                        prediction_type="survival", seed=seed, n_split=4, intercept=True, reg=.01)

        seed += 1
        shap_res.append(tmp)

    all_shap.append(shap_res)
np.save('results/shap_PBC_mean_CoxSig.npy', np.array(all_shap, dtype=object), allow_pickle=True)