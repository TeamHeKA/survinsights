...
# train
dynamic_deephit.train(tr_data_full, is_trained=False, ax=axs["A"], ckpt_dir="./dynamic_deephit_ckpt/MSK")
...
axs["B"].legend().remove()
plt.savefig('figures/MSK_DDH_train.pdf', bbox_inches="tight")
...
for pred_time in pred_times:
    ...
    for i in range(K):
        tmp = survshap_longi(model_explainer, i, sel_paths[i], sel_static_feature[i], np.array([pred_time]), 
                            prediction_type="med-survival", seed=seed, n_split=4, reg=.1)

        seed += 1
        shap_res.append(tmp)

    all_shap.append(shap_res)
np.save('results/shap_MSK_med_DDH.npy', np.array(all_shap, dtype=object), allow_pickle=True)