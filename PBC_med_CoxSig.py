...
pred_times = np.quantile(np.array(tte), quantile_pred_times) - 2 * dt
n_eval_times = 3
eval_times = []
...
axs[2].legend().remove()
plt.tight_layout()
plt.savefig('figures/PBC_Coxsig_train.pdf')
...
for pred_time in pred_times:
    ...
    for i in range(K):
        tmp = survshap_longi(model_explainer, i, sel_paths[i], sel_static_feature[i], pred_time=np.array([pred_time]), 
                                        prediction_type="med-survival", seed=seed, n_split=4, intercept=True, reg=.01)

        seed += 1
        shap_res.append(tmp)

    all_shap.append(shap_res)
np.save('results/shap_PBC_med_CoxSig.npy', np.array(all_shap, dtype=object), allow_pickle=True)