...
import iisignature
import torch
from examples.learner.coxprox import CoxProx
from examples.learner.utils import score, plot_loss_evolution
...
class CoxSignature():
    ...
    def train(self, path, surv_label, static_feat=None, plot_loss=False, ax=None):
        """
    ...
    def predict_survival(self, path, pred_times, static_feat=None):
        ...
        for j in np.arange(n_pred_times):
            ...
            # print("t_pred_id", t_pred_id)
            surv_pred_at_t = surv_preds[:, j, t_pred_id]
            # print("surv_pred_at_t", surv_pred_at_t)
            surv_pred_at_t[surv_pred_at_t == 0] = 1.
